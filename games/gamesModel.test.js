...
describe('gamesModel', () => {
  ...
  describe('remove handler', () => {
    it('returns false when an invalid id is passed', () => {
      const deletedGame = gamesModel.remove(-1);
      expect(deletedGame).toBe(false);
    });
    it('returns true when an valid id is passed', () => {
        const deletedGame = gamesModel.remove(1);
        expect(deletedGame).toBe(true);
      });
      it('removes the game from the list when a valid id is passed', () => {
        const games = gamesModel.get();
        expect(games).toHaveLength(3);
        gamesModel.remove(3);
